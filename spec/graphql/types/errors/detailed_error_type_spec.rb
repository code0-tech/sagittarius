...
RSpec.describe SagittariusSchema.types['DetailedError'] do
  ...
  describe '.resolve_type' do
    ...
    it 'resolves message errors' do
      expect(
        described_class.resolve_type({ message: 'message' }, {})
      ).to eq(Types::Errors::MessageErrorType)
    end

    it 'raises an error for invalid types' do
      expect { described_class.resolve_type(build(:user), {}) }.to raise_error 'Unsupported DetailedErrorType'
    end
  end
end
