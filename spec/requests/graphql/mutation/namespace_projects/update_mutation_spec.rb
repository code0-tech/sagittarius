...
RSpec.describe 'namespaceProjectsUpdate Mutation' do
  ...
  let(:mutation) do
    <<~QUERY
      mutation($input: NamespaceProjectsUpdateInput!) {
        namespaceProjectsUpdate(input: $input) {
          #{error_query}
          namespaceProject {
            name
            description
  ...
  context 'when user is a member of the namespace' do
    ...
    it 'updates namespace project' do
      ...
      expect(graphql_data_at(:namespace_projects_update, :namespace_project, :id)).to be_present

      project = SagittariusSchema.object_from_id(
        graphql_data_at(:namespace_projects_update, :namespace_project, :id)
      )
    ...
    context 'when namespace project name is taken' do
      ...
      it 'returns an error' do
        ...
        expect(graphql_data_at(:namespace_projects_update, :namespace_project)).to be_nil
        expect(
          graphql_data_at(:namespace_projects_update, :errors)
  ...
  context 'when user is not a member of the namespace' do
    it 'returns an error' do
      ...
      expect(graphql_data_at(:namespace_projects_update, :namespace_project)).to be_nil
      expect(graphql_data_at(:namespace_projects_update, :errors)).to include({ 'message' => 'missing_permission' })
    end
  end
end
