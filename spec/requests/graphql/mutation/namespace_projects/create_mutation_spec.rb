...
RSpec.describe 'namespaceProjectsCreate Mutation' do
  ...
  let(:mutation) do
    <<~QUERY
      mutation($input: NamespaceProjectsCreateInput!) {
        namespaceProjectsCreate(input: $input) {
          #{error_query}
          namespaceProject {
            id
            name
  ...
  context 'when user is a member of the namespace' do
    ...
    it 'creates namespace project' do
      ...
      created_project_id = graphql_data_at(:namespace_projects_create, :namespace_project, :id)
    ...
    context 'when namespace project name is taken' do
      ...
      it 'returns an error' do
        ...
        expect(graphql_data_at(:namespace_projects_create, :namespace_project)).to be_nil
        expect(
          graphql_data_at(:namespace_projects_create, :errors)
  ...
  context 'when user is not a member of the namespace' do
    it 'returns an error' do
      ...
      expect(graphql_data_at(:namespace_projects_create, :namespace_project)).to be_nil
      expect(graphql_data_at(:namespace_projects_create, :errors)).to include({ 'message' => 'missing_permission' })
    end
  end
end
