    ...
    context 'when organization role name is taken in another organization' do
      let(:other_organization) do
        create(:organization).tap do |o|
          create(:organization_role, organization: o, name: input[:name])
          create(:organization_project, organization: o, name: input[:name])
        end
      end
      ...
      it 'creates organization role' do
        ...
        expect(graphql_data_at(:organization_project_create, :organization_project, :id)).to be_present

        organization_project = SagittariusSchema.object_from_id(
          graphql_data_at(:organization_project_create, :organization_project, :id)
        )
