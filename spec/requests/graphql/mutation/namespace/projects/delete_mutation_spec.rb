...
RSpec.describe 'namespacesProjectsDelete Mutation' do
  ...
  let(:mutation) do
    <<~QUERY
      mutation($input: NamespacesProjectsDeleteInput!) {
        namespacesProjectsDelete(input: $input) {
          #{error_query}
          namespaceProject {
            id
            namespace {
  ...
  context 'when user is a member of the namespace' do
    ...
    it 'deletes namespace project' do
      ...
      expect(graphql_data_at(:namespaces_projects_delete, :namespace_project, :id)).to be_present

      expect(
        SagittariusSchema.object_from_id(
          graphql_data_at(:namespaces_projects_delete, :namespace_project, :id)
        )
      ).to be_nil
  ...
  context 'when user is not a member of the namespace' do
    it 'returns an error' do
      ...
      expect(graphql_data_at(:namespaces_projects_delete, :namespace_project)).to be_nil
      expect(graphql_data_at(:namespaces_projects_delete, :errors, :error_code)).to include('MISSING_PERMISSION')
    end
  end
end
