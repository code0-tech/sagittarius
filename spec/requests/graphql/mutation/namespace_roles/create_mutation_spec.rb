...
RSpec.describe 'namespaceRolesCreate Mutation' do
  ...
  let(:mutation) do
    <<~QUERY
      mutation($input: NamespaceRolesCreateInput!) {
        namespaceRolesCreate(input: $input) {
          #{error_query}
          namespaceRole {
            id
            name
  ...
  context 'when user is a member of the namespace' do
    ...
    it 'creates namespace role' do
      ...
      expect(graphql_data_at(:namespace_roles_create, :namespace_role, :id)).to be_present

      namespace_role = SagittariusSchema.object_from_id(
        graphql_data_at(:namespace_roles_create, :namespace_role, :id)
      )
    ...
    context 'when namespace role name is taken' do
      ...
      it 'returns an error' do
        ...
        expect(graphql_data_at(:namespace_roles_create, :namespace_role)).to be_nil
        expect(
          graphql_data_at(:namespace_roles_create, :errors)
  ...
  context 'when user is not a member of the namespace' do
    it 'returns an error' do
      ...
      expect(graphql_data_at(:namespace_roles_create, :namespace_role)).to be_nil
      expect(graphql_data_at(:namespace_roles_create, :errors)).to include({ 'message' => 'missing_permission' })
    end
  end
end
