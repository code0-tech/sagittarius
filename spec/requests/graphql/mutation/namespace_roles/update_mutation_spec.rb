...
RSpec.describe 'namespaceRolesUpdate Mutation' do
  ...
  let(:mutation) do
    <<~QUERY
      mutation($input: NamespaceRolesUpdateInput!) {
  ...
  context 'when user is a member of the namespace' do
    ...
    it 'updates namespace role' do
      ...
      expect(graphql_data_at(:namespace_roles_update, :namespace_role, :id)).to be_present

      namespace_role = SagittariusSchema.object_from_id(
        graphql_data_at(:namespace_roles_update, :namespace_role, :id)
      )
    ...
    context 'when namespace role name is taken' do
      ...
      it 'returns an error' do
        ...
        expect(graphql_data_at(:namespace_roles_update, :namespace_role)).to be_nil
        expect(
          graphql_data_at(:namespace_roles_update, :errors)
  ...
  context 'when user is not a member of the namespace' do
    it 'returns an error' do
      ...
      expect(graphql_data_at(:namespace_roles_update, :namespace_role)).to be_nil
      expect(graphql_data_at(:namespace_roles_update, :errors)).to include({ 'message' => 'missing_permission' })
    end
  end
end
