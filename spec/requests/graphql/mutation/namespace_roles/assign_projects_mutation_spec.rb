...
RSpec.describe 'namespaceRolesAssignProjects Mutation' do
  ...
  let(:mutation) do
    <<~QUERY
      mutation($input: NamespaceRolesAssignProjectsInput!) {
        namespaceRolesAssignProjects(input: $input) {
          #{error_query}
          projects {
            id
          }
  ...
  context 'when user has permission' do
    ...
    it 'assigns the given projects to the role' do
      ...
      project_ids = graphql_data_at(:namespace_roles_assign_projects, :projects, :id)
  ...
  context 'when user does not have permission' do
    it 'returns an error' do
      ...
      expect(graphql_data_at(:namespace_roles_assign_projects, :projects)).to be_nil
      expect(
        graphql_data_at(:namespace_roles_assign_projects, :errors)
