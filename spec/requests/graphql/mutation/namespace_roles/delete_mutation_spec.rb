...
RSpec.describe 'namespaceRolesDelete Mutation' do
  ...
  let(:mutation) do
    <<~QUERY
      mutation($input: NamespaceRolesDeleteInput!) {
        namespaceRolesDelete(input: $input) {
          #{error_query}
          namespaceRole {
            id
            name
  ...
  context 'when user is a member of the namespace' do
    ...
    it 'deletes namespace role' do
      ...
      expect(graphql_data_at(:namespace_roles_delete, :namespace_role, :id)).to be_present

      expect(
        SagittariusSchema.object_from_id(
          graphql_data_at(:namespace_roles_delete, :namespace_role, :id)
        )
      ).to be_nil
  ...
  context 'when user is not a member of the namespace' do
    it 'returns an error' do
      ...
      expect(graphql_data_at(:namespace_roles_delete, :namespace_role)).to be_nil
      expect(graphql_data_at(:namespace_roles_delete, :errors)).to include({ 'message' => 'missing_permission' })
    end
  end
end
