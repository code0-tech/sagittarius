...
RSpec.describe 'namespaceMembersInvite Mutation' do
  ...
  let(:mutation) do
    <<~QUERY
      mutation($input: NamespaceMembersInviteInput!) {
        namespaceMembersInvite(input: $input) {
          #{error_query}
          namespaceMember {
            id
            user {
  ...
  context 'when user has permission' do
    ...
    it 'creates namespace member' do
      ...
      expect(graphql_data_at(:namespace_members_invite, :namespace_member, :id)).to be_present

      namespace_member = SagittariusSchema.object_from_id(
        graphql_data_at(:namespace_members_invite, :namespace_member, :id)
      )
    ...
    context 'when target user is already a member' do
      it 'returns an error' do
        ...
        expect(graphql_data_at(:namespace_members_invite, :namespace_member)).to be_nil
        expect(
          graphql_data_at(:namespace_members_invite, :errors)
  ...
  context 'when user does not have permission' do
    it 'returns an error' do
      ...
      expect(graphql_data_at(:namespace_members_invite, :namespace_member)).to be_nil
      expect(graphql_data_at(:namespace_members_invite, :errors)).to include({ 'message' => 'missing_permission' })
    end
  end
end
