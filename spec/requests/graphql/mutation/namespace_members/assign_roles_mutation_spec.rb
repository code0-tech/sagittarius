...
RSpec.describe 'namespaceMembersAssignRoles Mutation' do
  ...
  let(:mutation) do
    <<~QUERY
      mutation($input: NamespaceMembersAssignRolesInput!) {
        namespaceMembersAssignRoles(input: $input) {
          #{error_query}
  ...
  context 'when user has permission' do
    ...
    it 'assigns the given roles to the member' do
      ...
      role_ids = graphql_data_at(:namespace_members_assign_roles, :namespace_member_roles, :id)
  ...
  context 'when user does not have permission' do
    it 'returns an error' do
      ...
      expect(graphql_data_at(:namespace_members_assign_roles, :namespace_member_roles)).to be_nil
      expect(
        graphql_data_at(:namespace_members_assign_roles, :errors)
