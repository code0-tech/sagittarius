...
RSpec.describe 'namespaceMembersDelete Mutation' do
  ...
  let(:mutation) do
    <<~QUERY
      mutation($input: NamespaceMembersDeleteInput!) {
        namespaceMembersDelete(input: $input) {
          #{error_query}
          namespaceMember {
            id
            user {
  ...
  context 'when user has permission' do
    ...
    it 'deletes namespace member' do
      ...
      expect(graphql_data_at(:namespace_members_delete, :namespace_member, :id)).to be_present

      expect(
        SagittariusSchema.object_from_id(
          graphql_data_at(:namespace_members_delete, :namespace_member, :id)
        )
      ).to be_nil
  ...
  context 'when user does not have permission' do
    it 'returns an error' do
      ...
      expect(graphql_data_at(:namespace_members_delete, :namespace_member)).to be_nil
      expect(graphql_data_at(:namespace_members_delete, :errors)).to include({ 'message' => 'missing_permission' })
    end
  end
end
