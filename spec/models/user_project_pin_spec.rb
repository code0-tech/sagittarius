...
RSpec.describe UserProjectPin do
  ...
  describe 'associations' do
    ...
    it { is_expected.to belong_to(:namespace).inverse_of(:user_project_pins) }
  ...
  describe 'validations' do
    ...
    it { is_expected.to validate_uniqueness_of(:priority).scoped_to(%i[user_id namespace_id]) }
    it { is_expected.to validate_uniqueness_of(:project_id).scoped_to(:user_id) }

    it 'is invalid when the namespace does not match the project namespace' do
      pin = build(:user_project_pin, namespace: create(:organization).ensure_namespace)

      expect(pin).not_to be_valid
      expect(pin.errors[:namespace]).to be_present
    end
  end
end
