...
RSpec.describe 'namespacesLicensesCreate Mutation' do
  ...
  let(:mutation) do
    <<~QUERY
      mutation($input: NamespacesLicensesCreateInput!) {
        namespacesLicensesCreate(input: $input) {
          #{error_query}
          namespaceLicense {
            id
            namespace {
  ...
  context 'when user is a member of the namespace' do
    ...
    it 'creates namespace license' do
      ...
      expect(graphql_data_at(:namespaces_licenses_create, :namespace_license, :id)).to be_present

      namespace_license = SagittariusSchema.object_from_id(
        graphql_data_at(:namespaces_licenses_create, :namespace_license, :id)
      )
    ...
    context 'when license is invalid' do
      ...
      it 'returns an error' do
        ...
        expect(graphql_data_at(:namespaces_licenses_create, :namespace_license)).to be_nil
        expect(
          graphql_data_at(:namespaces_licenses_create, :errors, :details)
  ...
  context 'when user is not a member of the namespace' do
    it 'returns an error' do
      ...
      expect(graphql_data_at(:namespaces_licenses_create, :namespace_license)).to be_nil
      expect(graphql_data_at(:namespaces_licenses_create, :errors, :error_code)).to include('MISSING_PERMISSION')
    end
  end
end
