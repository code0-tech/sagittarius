  ...
  def execute
    ...
    transactional do |t|
      params.each do |param, value|
        ...
        if setting.blank?
          t.rollback_and_return! ServiceResponse.error(message: 'Invalid setting', error_code: :invalid_setting)
        end

        setting.value = value
        unless setting.save
          t.rollback_and_return! ServiceResponse.error(message: 'Failed to update setting',
                                                       error_code: :invalid_setting, details: setting.errors)
        end
