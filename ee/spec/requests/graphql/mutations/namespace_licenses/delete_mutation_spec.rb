...
RSpec.describe 'namespaceLicensesDelete Mutation' do
  ...
  let(:mutation) do
    <<~QUERY
      mutation($input: NamespaceLicensesDeleteInput!) {
        namespaceLicensesDelete(input: $input) {
          #{error_query}
          namespaceLicense {
            id
            namespace {
  ...
  context 'when user is a member of the namespace' do
    ...
    it 'deletes namespace license' do
      ...
      expect(graphql_data_at(:namespace_licenses_delete, :namespace_license, :id)).to be_present

      namespace_license = SagittariusSchema.object_from_id(
        graphql_data_at(:namespace_licenses_delete, :namespace_license, :id)
      )
  ...
  context 'when user is not a member of the namespace' do
    it 'returns an error' do
      ...
      expect(graphql_data_at(:namespace_licenses_delete, :namespace_license)).to be_nil
      expect(graphql_data_at(:namespace_licenses_delete, :errors)).to include({ 'message' => 'missing_permission' })
    end
  end
end
